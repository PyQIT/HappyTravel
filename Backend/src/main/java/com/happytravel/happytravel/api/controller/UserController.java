...
public class UserController {
    ...
    @GetMapping("/signin")
    public User logIN(@RequestParam String login, @RequestParam String pass){
        User user = userService.logIn(login, pass);
        return user;
    }
    @GetMapping("/signup")
    public int signUp(@RequestParam String login, @RequestParam String pass, @RequestParam String userType){
                if(userService.checkLogins(login)!=null) return -1;
                else {
                    Long userid = userService.getMaxId()+1;
                    int result = 0;
                    result = userService.signUp(userid, login, pass, userType);
                    if (result == 0) return -2;
                    Long personId = personService.getMaxId() + 1;
                    result = personService.insertPerson(personId, "osoba@cos.com", "Krzysztof", 11111111111l, "123123123", "Bielski", userid);
                    if (result == 0) return -2;
                    if(userType == "Klient"){
